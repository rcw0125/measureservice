(function ($) {
    ...
    var crudFunctions = {
        ...
        exportremote:'<div class="crud btn-group btn-group-sm"><button class="btn btn-info type="button" onclick="%s(%s,%s,%s,%s);"><i class="glyphicon glyphicon-export icon-share"></i></button></div>',
        importexcel:'<div class="crud btn-group btn-group-sm"><button class="btn btn-warning type="button" onclick="%s(%s,%s,%s,%s);"><i class="glyphicon glyphicon-import icon-share"></i></button></div>',
        ...
        copy:'<div class="crud btn-group btn-group-sm"><button class="btn btn-success type="button" onclick="%s();"><i class="glyphicon glyphicon-copy icon-share"></i></button></div>',
        combine: 'COMBINE',
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        btnFuncs:'addFunction,removeFunction,remoteExportExcel'
    });
        ...
        if (this.options.showCrud) {
            ...
            if (!$export.length){
            	...
            	for(var i=0;i<btnsDefine.length;i++){
            		btns = btns + sprintf(crudFunctions[btnsDefine[i]],btnsFuncDefine[i],'\'' + this.$el[0].id + '\'','\'' + this.options.moduleclass + '\'','\'' + this.options.datatablename + '\'','\'' + this.options.queryForm + '\'');
            	}
